...
impl SecretKey {
    ...
    pub(crate) fn gen_b0(seed: [u8; 32]) -> [Polynomial<i16>; 4] {
        let (f, g, capital_f, capital_g) = ntru_gen(seed);
    ...
    pub(crate) fn from_b0(b0: [Polynomial<i16>; 4]) -> Self {
        ...
        let sigma = FalconVariant::from_n().parameters().sigma;
    ...
    pub fn from_bytes(byte_vector: &[u8]) -> Result<Self, FalconDeserializationError> {
        ...
        if n != FalconVariant::from_n().parameters().n {
...
pub fn sign(m: &[u8], sk: &SecretKey) -> Signature {
    ...
    let params = FalconVariant::from_n().parameters();
...
mod test {
    ...
    fn test_falcon512_test_vector() {
        ...
        let sig = Signature {
            ...
            s: compress(
                ...
                (FalconVariant::from_n().parameters().sig_bytelen - 41) * 8,
            )
            .unwrap(),
        };
    ...
    fn test_secret_key_field_element_serialization() {
        ...
        for polynomial_index in [0, 1, 2] {
            let width = SecretKey::field_element_width(512, polynomial_index);
            for _ in 0..100000 {
